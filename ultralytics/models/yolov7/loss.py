...
from .common import bbox_iou, box_iou, xywh2xyxy
from ultralytics.utils.torch_utils import is_parallel

HYP = os.getenv("HYP", "./data/hyp.scratch.p5.yaml")
...
def compute_loss(model):
    ...
    if "loss_ota" not in hyp or hyp["loss_ota"] == 1:
        return ComputeLossOTA(model, hyp=hyp)
    else:
        return ComputeLoss(model, hyp=hyp)

class ComputeLoss:
    # Compute losses
    def __init__(self, model, autobalance=False, hyp=None):
